...
class SisuAPI {
    ...
    public JsonObject getJsonObjectFromApi() {
        ...
        try {
            switch (url) {
                case 1:
                    correctUrl = new URL("https://sis-tuni.funidata.fi/kori/api/module-search?"
                            + "curriculumPeriodId=uta-lvv-2021&universityId=tuni-university"
                            + "-root-id&moduleType=DegreeProgramme&limit=1000");
                    break;
                case 2:
                    correctUrl = new URL(String.format("https://sis-tuni.funidata.fi/kori/api/modules/" + groupId));
                    break;
                case 3:
                    correctUrl = new URL(String.format("https://sis-tuni.funidata.fi/kori/api/modules/by-group-id?groupId="
                            + groupId + "&universityId=tuni-university-root-id"));
                    break;
                case 4:
                    correctUrl = new URL(
                            String.format("https://sis-tuni.funidata.fi/kori/api/course-units/by-group-id?groupId="
                                    + groupId + "&universityId=tuni-university-root-id"));
                    break;
            }
