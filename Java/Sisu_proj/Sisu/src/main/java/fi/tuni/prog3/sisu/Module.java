    ...
    while (iter.hasNext()) {
      ...
      switch (type) {
        ...
        case "CourseUnitRule":
          ...
          int minCredits = courseObj.getAsJsonObject("credits").getAsJsonPrimitive("min").getAsInt();

          if (courseObj.getAsJsonObject("name").getAsJsonPrimitive("fi") == null) {
            courseName = courseObj.getAsJsonObject("name").getAsJsonPrimitive("en").getAsString();
          } else {
            courseName = courseObj.getAsJsonObject("name").getAsJsonPrimitive("fi").getAsString();
          }

          if (!courseObj.get("outcomes").isJsonNull()) {
            if (courseObj.getAsJsonObject("outcomes").get("fi") == null) {
                outcomes = courseObj.getAsJsonObject("outcomes").getAsJsonPrimitive("en").getAsString();
            } else {
                outcomes = courseObj.getAsJsonObject("outcomes").getAsJsonPrimitive("fi").getAsString();
            }
          } else {
